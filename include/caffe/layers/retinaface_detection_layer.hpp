#ifndef CAFFE_RETINAFACE_DETECTION_LAYER_HPP_
#define CAFFE_RETINAFACE_DETECTION_LAYER_HPP_
...
class RetinaFaceDetectionLayer : public Layer<Dtype> {
public:
    explicit RetinaFaceDetectionLayer(const LayerParameter& param)
        : Layer<Dtype>(param) {
        cfg_.clear();
        ...
        anchors_fpn_.clear();
        auto anchors_fpn = generate_anchors_fpn(false, cfg_);
        for (int i = 0; i < feature_stride_fpn_.size(); ++i) {
            std::string key = "stride" + std::to_string(feature_stride_fpn_[i]);
    ...
    virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top);
    virtual inline const char* type() const { return "RetinaFaceDetection"; }
...
private:
    ...
    std::vector<AnchorCfg> cfg_;
    std::vector<int> feature_stride_fpn_{32, 16, 8};
    std::unordered_map<std::string, std::vector<AnchorBox>> anchors_fpn_;
    std::unordered_map<std::string, int> num_anchors_;
};
